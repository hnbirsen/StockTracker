...
- [x] `POST /billing/verify-purchase` endpoint'i — mobil client, kullanıcı satın almayı App Store/Play Store'da tamamladıktan sonra `{userId, platform, transactionIdOrToken, subscriptionOrProductId?}` gönderir; Billing Service ilgili store'un **server-to-server** API'sine karşı doğrular (Apple: App Store Server API `GET /inApps/v1/transactions/{id}`; Google: Play Developer API `purchases.subscriptions.get`) — doğrulanırsa `UserSubscriptions` kaydı oluşturulur/güncellenir ve kullanıcı **anında** Premium'a yükseltilir (webhook'u beklemeden)
- [x] `POST /billing/webhooks/apple` — App Store Server Notifications V2, JWS-signed payload, imza doğrulaması `AppleJwsVerifier` ile
- [x] `POST /billing/webhooks/google` — Real-time Developer Notifications (Cloud Pub/Sub push), OIDC bearer token doğrulaması `GoogleOidcTokenValidator` ile (Google'ın gerçek, canlı JWKS endpoint'ine karşı — Apple'ın aksine burada sabit/gömülü bir sertifikaya gerek yok)
...
- [x] **Bu doğrulama sırasında gerçek bir hata bulundu ve düzeltildi**: `AppleStoreSettings.IsConfigured`/`GooglePlaySettings.IsConfigured`, `.env`'deki doldurulmamış `REPLACE_WITH_ENV` placeholder'ını "yapılandırılmış" sayıyordu (yalnızca null/boş kontrolü vardı) — bu yüzden `POST /billing/verify-purchase` gerçek bir anahtar olmadan geçersiz Base64 ile `FormatException` fırlatıp 500 dönüyordu, "yapılandırılmamışsa nazikçe atla" tasarımı bu noktada bozulmuştu (Faz 3.3'te SendGrid/FCM için eklenen aynı kontrol burada unutulmuştu). `IsSet()` yardımcı metoduna `REPLACE_WITH_ENV` kontrolü eklenip düzeltildi, regresyon testleri (`SettingsConfigurationTests`) eklendi.
