...
**`POST /billing/verify-purchase`** — mobil client'ın tamamladığı satın almayı doğrular, doğrulanırsa kullanıcıyı **hemen** Premium'a yükseltir (webhook'u beklemeden — satın alma anında erişim açılmalı, kullanıcı deneyimi beklentisi). Plan seçimi şu an her zaman Premium: `Plans.AppStoreProductId`/`PlayStoreProductId` gerçek store ürünleri oluşturulana kadar `null` olduğundan productId→plan eşlemesi henüz güvenilir değil — MVP'nin iki-planlı (Free/Premium) yapısıyla tutarlı, gerçek store ürünleri tanımlanınca genişletilebilir.
