...
public class OyshoStockCheckServiceTests
{
    private readonly Mock<IOyshoStockApiClient> _apiClient = new();

    private const string ProductUrl = "https://www.oysho.com/tr/test-urun-l36613922";

    private OyshoStockCheckService CreateSut() =>
        new(_apiClient.Object, Mock.Of<ILogger<OyshoStockCheckService>>());
    ...
    public async Task CheckAsync_WhenNoStoreId_CallsOnlineCheckAndReturnsInStock()
    {
        ...
        result.ScraperSource.Should().Be("oysho-online-api");
    ...
    public async Task CheckAsync_WhenStoreIdAndBrandSpecificStoreIdPresent_CallsStoreCheck()
    {
        ...
        _apiClient.Setup(c => c.CheckStoreStockAsync(command.ProductCode, command.Size, "2371", ProductUrl, It.IsAny<CancellationToken>()))
        ...
        result.ScraperSource.Should().Be("oysho-store-api");
    ...
    {
        // Online kontrolde Oysho'nın API'si sayısal miktar vermiyor (bkz. OyshoStockApiClient) —
